...
public class FarmAndCharmClientUtil {
    public static <T extends BlockEntity> void renderBlock(BlockState state, PoseStack matrices, MultiBufferSource vertexConsumers, T entity){
        ...
        Minecraft.getInstance().getBlockRenderer().renderSingleBlock(state, matrices, vertexConsumers, FarmAndCharmClientUtil.getLightLevel(level, entity.getBlockPos()), OverlayTexture.NO_OVERLAY);
    }
    ...
    public static <T extends BlockEntity> void renderItem(ItemStack stack, PoseStack matrices, MultiBufferSource vertexConsumers, T entity){
        Level level = entity.getLevel();
        if(level == null) return;
        Minecraft.getInstance().getItemRenderer().renderStatic(stack, ItemDisplayContext.GUI, FarmAndCharmClientUtil.getLightLevel(level, entity.getBlockPos()),
